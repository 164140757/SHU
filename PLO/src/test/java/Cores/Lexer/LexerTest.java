...
class LexerTest {
    ...
    void outPutTypeTest(){
        ...
        for (int i = 0; i < Objects.requireNonNull(listOfFiles).length; i++) {
            if (listOfFiles[i].isFile()) {
                ...
                Writer writer = null;
                try {
                    writer = new Writer(filePath + "\\Name\\" + file.getName().split("\\.")[0] + "Result.txt");
                } catch (IOException e) {
                    e.printStackTrace();
                }
                Lexer lexer = new Lexer();
    ...
    void specificFileTest() throws IOException {
        ...
        HashMap<Word, Integer> statisticMap = lexer.getIDNums();
        Vector<String> context = new Vector<>();
        statisticMap.forEach((k, v) -> context.addAll(Arrays.asList("(", k.lexeme,": ", Integer.toString(v), ")", "\n")));
        assert writer != null;
        writer.write(context);
    }
}