 ...
 * @LastEditTime: 2020-05-15 11:33:02
...
public class ParserTest {
    ...
    void findFirstTest2() throws Exception {
        ...
        parser.getFirst();
        HashMap<NonTerminal, Set<Token>> first = parser.first;
        assertEquals(new HashSet<>(Arrays.asList(new Token("("),new Token("i"))), 
        first.get(new NonTerminal("E")), "first set fails.");
        
        assertEquals(new HashSet<>(Arrays.asList(new Token("+"),new Token("#"))), 
        first.get(new NonTerminal("A")), "first set fails.");

        assertEquals(new HashSet<>(Arrays.asList(new Token("("),new Token("i"))), 
        first.get(new NonTerminal("T")), "first set fails.");
    }
    ...
    void followTest() throws Exception {
        ...
        assertEquals(new HashSet<>(Arrays.asList(new Token("!"))), 
        follow.get(new NonTerminal("S")), "first set fails.");
        
        assertEquals(new HashSet<>(Arrays.asList(new Token("a"),new Token("!"),new Token("c"))), 
        follow.get(new NonTerminal("A")), "first set fails.");

        assertEquals(new HashSet<>(Arrays.asList(new Token("!"))), 
        follow.get(new NonTerminal("D")), "first set fails.");
    }
    ...
    void selectTest() throws Exception {
        ...
        parser.getFollow();
        parser.getSelect();
        HashMap<Production,Set<Token>> select = parser.select;
        assertEquals(new HashSet<>(Arrays.asList(new Token("b"),new Token("!"),new Token("a"))), select.get(new Production(
            new NonTerminal("S"),new Vector<Token>(Arrays.asList(new NonTerminal("A"),new NonTerminal("B")))
        )), "select test fails.");
        assertEquals(new HashSet<>(Arrays.asList(new Token("a"),new Token("!"),new Token("c"))), select.get(new Production(
            new NonTerminal("A"),new Vector<Token>(Arrays.asList(new Token("#")))
        )), "select test fails.");
    }
