 ...
 * @LastEditTime: 2020-05-13 20:16:26
...
public class ParserTest {
    ...
    void followTest() throws Exception {
        ...
        parser = new Parser(grammar);
        parser.getFollow();
        HashMap<Token, Set<Token>> follow = parser.follow;
        assertEquals(new HashSet<>(Arrays.asList(new Token("!"))), 
        follow.get(new NonTerminal("E")), "first set fails.");
        
        assertEquals(new HashSet<>(Arrays.asList(new Token("a"),new Token("!"),new Token("!"))), 
        follow.get(new NonTerminal("A")), "first set fails.");

        assertEquals(new HashSet<>(Arrays.asList(new Token("("),new Token("i"))), 
        follow.get(new NonTerminal("T")), "first set fails.");
    }
