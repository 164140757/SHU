...
class LexerTest {
        ...
        for (int i = 0; i < Objects.requireNonNull(listOfFiles).length; i++) {
            if (listOfFiles[i].isFile()) {
                ...
                statisticMap.forEach((k, v) -> {
                    context.addAll(Arrays.asList("(", k.name,": ", Integer.toString(v), ")", "\n"));
                });
                assert writer != null;
    ...
    void specificFileTest() throws IOException {
        ...
        HashMap<Word, Integer> statisticMap = lexer.getIDNums();
        Vector<String> context = new Vector<>();
        statisticMap.forEach((k, v) -> context.addAll(Arrays.asList("(", k.name,": ", Integer.toString(v), ")", "\n")));
        assert writer != null;
        writer.write(context);
    }
}
