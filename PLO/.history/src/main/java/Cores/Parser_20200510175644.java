...
public class Parser {
    ...
    void getFirst(Token t) {
        ...
        for (Vector<Token> tokens : production.target) {
            ...
            // else{      nonTerminal and can deduce empty      go next     boolean next =
            // true;     int index = 0;     while(next && index < tokens.size()){
            // next = false;          should make sure their is no left recursion
            // Production pY =  productions.get(tokens.get(index));          place all
            // nonempty (Tokens context != "#") to first     } }
        }
    }
