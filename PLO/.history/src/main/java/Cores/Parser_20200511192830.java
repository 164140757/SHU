 ...
 * @LastEditTime: 2020-05-11 19:28:21
...
/**
 * Recursively check NonTerminal t's targets, and find the NonTerminals and add to
 * @param production
 * @param firstSet
 */
    private void checkNonTerminal(Production production,Set<Token> firstSet) {
        ...
        for (Vector<Token> tokens : production.target) {
            for (Token token : tokens) {
                ...
                if (isTer(token)||token.context.equals("#")) {
                    // the token deduces terminal or empty add to firstSet
                    firstSet.add(token);
