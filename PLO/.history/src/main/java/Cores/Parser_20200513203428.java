...
public class Parser {
    ...
    Production parse(final Token t) {
 ...
 * @return has empty?
 */
    private boolean checkNonTerminal(final Production production, final Set<Token> tmpFirstSet) {
        // firstSet(not cleaned)
        final HashSet<Token> firstSet = new HashSet<>();
        ...
        for (final Vector<Token> tokens : production.target) {
            ...
            for (final Token token : tokens) {
                ...
                if (isTer(token)) {
                    ...
                    final Set<Token> tmp = first.get(token.toNonTerminal());
                    if (tmp != null) {
                        ...
                        final Production p_ = productions.get(token.toNonTerminal());
    ...
    public void getFollow() {
        ...
        do{
            ...
            for (final Entry<Token,Production> e : productions) {
                            ...
                            Token t = production.get(i);
                             ...
                             if (keySet.contains(t)) {
                                 ...
                                 if(i==production.size()-1){
                                    ...
                                    // record
                                    followSetChanged.put(t,set.addAll(follow.get(k)));
                                    follow.put(t,set);
                                 }
                                 else{
                                     //terminal
                                     ...
                                     if(isTer(n)){
                                        ...
                                        // record
                                        followSetChanged.put(t,set.add(n));
                                        follow.put(t,set);
                                     }
                                     else{
                                        Set<Token> set = follow.get(t);
                                        ...
                                        // record
                                        followSetChanged.put(t,set.addAll(first.get(n)));
                                        follow.put(t,set);
                                     }
                                 }
                             }
