 ...
 */
    private boolean checkNonTerminal(Production production, Set<Token> tmpFirstSet) {
        ...
        // firstSet for all productions starting with the same index
        // and tmpFirstSet for only one production
            ...
            for (Token token : tokens) {
                ...
                if (isTer(token)) {
                    ...
                    if (tmp != null) {
                        ...
                        firstSet.addAll(tmp);
                         // check if it is empty, yes -> go on, no-> break;
                         if(!(tmpFirstSet.size()==1&&tmpFirstSet.contains(new Token("#")))){
                            // clean tmp
                           tmpFirstSet.clear();
                           break;
                       }
    ...
    private void checkAndClean(Set<Token> tmpFirstSet) {
         // check if it is empty, yes -> go on, no-> break;
         if(!(tmpFirstSet.size()==1&&tmpFirstSet.contains(new Token("#")))){
            // clean tmp
           tmpFirstSet.clear();
