...
public class Production {
     ...
     * @param productions String representation of the productions, like "A BaMC;M
     *                    Bd; B #"
     * @return
     */
        ...
        res.forEach(production -> {
            ...
            for (Vector<Token> ts : target) {
                for (Token t : ts) {
                    ...
                    if (indexes.contains(new NonTerminal(context))) {
                        t.type = "NonTerminal";
                    }
                }
            }
    ...
    public static void addDefinition(String toAdd, List<Production> addToList, Class<? extends Terminal> defineClass)
            throws InstantiationException, IllegalAccessException {
        char[] array = toAdd.toCharArray();
        for (int i = 0; i < array.length; i++) {
            Terminal t = null;
            // translate a symbol to a specific class
            try {
                t = defineClass.getDeclaredConstructor(String.class, String.class).newInstance();
